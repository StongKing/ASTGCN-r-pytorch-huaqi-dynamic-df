...
def first_order_graph(W):
    """compute \tilde{D}^{-0.5}\tilde{W}\tilde{D}^{-0.5}
    """
    assert W.shape[0] == W.shape[1]
    W_with_self_loops = torch.tensor(W) + torch.eye(W.shape[0])
    # 计算度矩阵 D
    tD = torch.diag(W_with_self_loops.sum(dim=1))

    # 计算 D^(-1/2)
    D_inv_sqrt = torch.diag(1 / torch.sqrt(tD))
    D_inv_sqrt = torch.diag(D_inv_sqrt)
    # 计算归一化邻接矩阵
    W_normalized = torch.mm(D_inv_sqrt, torch.mm(W_with_self_loops, D_inv_sqrt))
    return W_normalized
...
def predict_and_save_results_mstgcn(net, data_loader, data_A_t, data_target_tensor, global_step, metric_method, _mean, _std,
    ...
    with torch.no_grad():
        ...
        # per_sample_time_sums: (B, T)
        per_sample_time_sums = prediction.sum(axis=1)
        # mean_time_sums: (T,)
        mean_time_sums = per_sample_time_sums.mean(axis=0)
        print("Prediction: 每个时刻所有节点单车数的样本平均值（t=0..T-1）：")
        for t, val in enumerate(mean_time_sums):
            print(f"t={t}: mean total across nodes = {val:.2f}")
        excel_list.extend([mae, rmse, mape])
        print(excel_list)
